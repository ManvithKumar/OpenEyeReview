...
[x: string]: any;
  ...
  canNavigateToStep(stepIndex: number): boolean {
    if (stepIndex === 1) return this.isPostNameFilled();
    if (stepIndex === 2) return this.isCategoryFilled();
    return false;
  }
  ...
  isPostNameFilled()
  {
    if(this.form.controls.postName.invalid || this.form.controls.description.invalid || this.form.controls.AdditionalName.invalid)
    {
      return false
    }
    else{
  ...
  isCategoryFilled()
  {
    if(this.form.controls.category.invalid || this.form.controls.label.invalid || this.form.controls.sublabel.invalid){
      return false
    }
    return true;
  }
  ...
  next() {
   ...
   if (this.canNavigateToStep(nextIndex)) {
    this.stepper.next();
    this.currentStepIndex = nextIndex; // Update index manually
  }
  else{
    Swal.fire({ toast: true, position: 'center', showConfirmButton: true,title: 'Enter All Fields!', text: 'Kindly ensure all required fields are filled in before proceeding.', icon: 'warning'});
  }
  }
  ...
  prev() {
    if (this.currentStepIndex > 0) {
      ...
      this.currentStepIndex--; // Update index manually
    }
  }
